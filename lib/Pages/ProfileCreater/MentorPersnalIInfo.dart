...
class MentorPersnalIInfo extends StatefulWidget {
  PageController controller;
  int slideIndex;
  String textLang="";
  MentorPersnalIInfo(this.controller,this.slideIndex);

  @override
  _MentorPersnalIInfoState createState() => _MentorPersnalIInfoState();
}

class _MentorPersnalIInfoState extends State<MentorPersnalIInfo> {
  var lang=[];
  var Areas=[];
  List<String> gander = ["Gender","Male","Female","Others"];
  List<String> Mentorship = ["STEM","Social Sciences","Humanities","Languages","College Applications","Career Advice","Life Coaching"];
  List<PopupMenuItem> GenderpopList = [];
  List<PopupMenuItem> MentorshippopList = [];
  String textGenderpopList="";
  String textMentorshipList="";
  String textLang="";
  final GlobalKey _menuKey = new GlobalKey();
  final GlobalKey _menuKey1 = new GlobalKey();
  ...
  void initState() {
    super.initState();
    context.findAncestorStateOfType<_MentorPersnalIInfoState>();
    final FirebaseDatabase database = FirebaseDatabase(app: app);
    ref = database.reference();
    if(model.personl==null){
      model.personl=PersonlBean();
    }
    Mentorship.asMap().forEach((i, value) {
      MentorshippopList.add(
          PopupMenuItem(
            child: Row(
                          ...
                          Expanded(
                            child:Column(
                              ...
                              children: [
                                ...
                                Divider(height: 3,thickness:3 ,color: Colors.white,),
                                Text("Preferred language",style: TextStyle(color: Colors.white),)
                              ],
                            ),
                          )
                        ],),
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Expanded(
                                        child: GestureDetector(
                                          child: Text(model.personl.mentorship!=null?model.personl.mentorship:textMentorshipList,style: TextStyle(color: Colors.white),),onTap: (){
                                          dynamic state = _menuKey1.currentState;
                                          state.showButtonMenu();
                                        },),
                                      ),
                                      PopupMenuButton(
                                        ...
                                        onSelected: (selectedDropDownItem){
                                          setState(() {
                                            model.personl.mentorship=selectedDropDownItem;
                                          });
                                        },
                                        itemBuilder: (BuildContext context) =>MentorshippopList,
                                        tooltip: "Tap me to select a number.",
                                      ),
                                    ],),
                                ),
                                Divider(height: 3,thickness:3 ,color: Colors.white,),
                                Text("Primary Mentorship Area",style: TextStyle(color: Colors.white),)
                              ],
                            ),
                          )
                        ],),
                          ...
                          children: [
                            Container(
                              child:RaisedButton(
                                ...
                                onPressed:(){
                                  model.personl.firestName=firstnameController.text.toString();
                                  model.personl.lastName=lastnameController.text.toString();
                                  print(model.toJson());
  ...
  Future<void> uploadToFirebase(File file) async {
    ...
    userdata.profile_img = "gs://squghted.appspot.com/images/${filePath}";
